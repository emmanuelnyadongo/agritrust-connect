                ...
                return (
                  <div
                    ...
                    className={`relative border-l-2 py-4 pl-6 ${
                      i < negotiation.offers.length - 1 ? '' : ''
                    } ${isFarmer ? 'border-primary/40' : 'border-info/40'}`}
                  >
                    {/* Dot */}
                    <div
                      className={`absolute -left-[5px] top-5 h-2 w-2 rounded-full ${
                        isFarmer ? 'bg-primary' : 'bg-info'
                      }`}
                    />
                    ...
                    <div className="flex items-baseline justify-between">
                      <div>
                        ...
                        <span className="ml-2 text-xs text-muted-foreground">
                          {new Date(offer.timestamp).toLocaleDateString('en-ZW', {
                            day: 'numeric',
                            month: 'short',
                            hour: '2-digit',
                            minute: '2-digit',
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  placeholder={`e.g. ${negotiation.systemGuidance.toFixed(2)}`}
        ...
        <div className="space-y-6">
          <section className="rounded border border-border p-4">
            <h3 className="mb-3 font-heading text-sm font-semibold text-foreground">
              System Price Guidance
            </h3>
            <p className="font-heading text-2xl font-semibold text-primary">
              ${negotiation.systemGuidance.toFixed(2)}/kg
            </p>
            <p className="mt-2 text-xs text-muted-foreground">
              Based on 30-day median of confirmed transactions for this produce type,
              quality grade, and region. This is guidance only — both parties decide
              the final price.
            </p>
          </section>
            ...
            <dl className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <dd className="text-foreground">
                  {lastOffer?.from === 'farmer' ? negotiation.farmer.name : negotiation.buyer.name}
                </dd>
              </div>
              <div className="flex justify-between">
                ...
                <dd className={`font-medium ${Number(guidanceDiff) > 0 ? 'text-warning' : 'text-primary'}`}>
                  {Number(guidanceDiff) > 0 ? '+' : ''}{guidanceDiff}%
                </dd>
              </div>
              <div className="flex justify-between">
          ...
          <section className="rounded border border-border p-4">
            ...
            <ul className="space-y-2 text-xs text-muted-foreground">
              <li>• Based on {Math.floor(Math.random() * 20 + 15)} similar transactions in this region</li>
          ...
          <div className="rounded border border-border bg-muted/30 p-4">
            <p className="text-xs text-muted-foreground">
              <span className="font-medium text-foreground">Note:</span> AgriTrust does not
              set prices. The guidance reflects market evidence. You are free to agree at
              any price both parties accept.
            </p>
          </div>
        </div>
      </div>
